...
function parseDeductions(lineItems) {
  ...
  for (const item of lineItems) {
    ...
    if (full.includes('family pack')) {
      // Haal kleuren uit properties: Kleur 1, Kleur 2, etc.
      const colorProps = props.filter(p => p.name && p.name.toLowerCase().includes('kleur') || (p.name && p.name.toLowerCase().includes('color')));
